...
import { cssInterviewQuestions } from './cssInterviewQuestions';
import { highlightCss } from '../../../lib/highlightCss';
import Seo from '../../../components/Seo';

const categories = Array.from(new Set(cssInterviewQuestions.map((q) => q.category)));
...
const CssInterviewQuestionsPage: React.FC = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return cssInterviewQuestions.filter((item) => {
  ...
  return (
    <article>
      <Seo
        title="100 CSS Interview Questions"
        description="100 CSS interview questions with detailed explanations and code examples, organized by category and difficulty - from basics to advanced topics."
      />
      ...
      <FadeIn immediate y={12} delay={0.15} className="mb-8 space-y-4">
        ...
        <p className="text-xs text-fg/40">
          Showing {filtered.length} of {cssInterviewQuestions.length} questions
        </p>
      </FadeIn>
                    ...
                    <div className="px-5 pb-5">
                      ...
                      <pre className="rounded-lg bg-bg border border-fg/10 p-4 overflow-x-auto">
                        <code
                          ...
                          dangerouslySetInnerHTML={{ __html: highlightCss(item.example) }}
                        />
                      </pre>
                    </div>
                  )}
      ...
      <div className="flex items-center justify-between mt-14 pt-8 border-t border-fg/10">
        <Link
          to="/interview-prep/css/variables-best-practices"
...
export default CssInterviewQuestionsPage;
